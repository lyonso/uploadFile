...
import webRtcPlayer from '../../public/webRtcPlayer'
...
function setupHtmlEvents() {
  ...
  if (encoderParamsSubmit !== null) {
    encoderParamsSubmit.onclick = function(event) {
      ...
      const targetBitrate = document.getElementById('encoder-target-bitrate-text').value * 1000
      const maxBitrate = document.getElementById('encoder-max-bitrate-text').value * 1000
      ...
      const fillerData = document.getElementById('encoder-filler-data-tgl').checked ? 1 : 0
  ...
  if (webrtcParamsSubmit !== null) {
    webrtcParamsSubmit.onclick = function(event) {
      const degradationPref = document.getElementById('webrtc-degradation-pref').value
...
function onWebRtcAnswer(webRTCData) {
  ...
  webRtcPlayerObj.latencyTestTimings.OnAllLatencyTimingsReady = function(timings) {
    ...
    const uePixelStreamLatency = timings.UEPreEncodeTimeMs == 0 || timings.UEPreCaptureTimeMs == 0 ? '???' : timings.UEPostEncodeTimeMs - timings.UEPreCaptureTimeMs
    const captureLatency = timings.UEPostCaptureTimeMs - timings.UEPreCaptureTimeMs
    ...
    const browserSendLatency = latencyExcludingDecode - networkLatency - ueLatency
...
function setupNormalizeAndQuantize() {
  ...
  if (playerElement && videoElement.length > 0) {
    ...
    if (playerAspectRatio > videoAspectRatio) {
      ...
      const unquantizeAndDenormalizeUnsigned = (x, y) => {
...
function connect() {
  ...
  ws = new WebSocket('ws://10.8.46.81:8099')
