<template>
  <div :class='isOpen? "rightPanel ":"rightPanel close"'>
    ...
    <div class='panel_Top'>
      <div v-for='(item,index) in list' :key='item.value' :style='item.img' class='item' @click='openDialog(index)'>
        <div>{{ item.name }}</div>
<!--        <div style='position: absolute;right: 40px;top: 5px;color: rgba(22, 173, 223, 1);cursor: pointer' v-if='index>1'-->
<!--        >查看-->
<!--        </div>-->
        <div>{{ item.value }}</div>
      </div>
    ...
    <div class='panel_Center'>
      <img src='../../../../assets/BIMIndex_icon5.png' @click='changeBIMIndexStatus'>
      {{ text }}
    </div>
...
export default {
  ...
  methods: {
    ...
    changeInfoShow() {
      EventBus.$emit('changeInfoShow', 0)
    },
    openDialog(type) {
      switch (type) {
        case 2:
          EventBus.$emit('changeInfoShow', this.modelStatus)
          this.modelStatus = (this.modelStatus === 0 ? 1 : 0)
          break
        case 3:
          this.modelStatus1 = !this.modelStatus1
          EventBus.$emit('modelShow', this.modelStatus1)
      }
    },
    changeBIMIndexStatus() {
      this.$store.commit('security/SET_BIMDETAILSSTATUS', 2)
    }
  }
}
</script>
